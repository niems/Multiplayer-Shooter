...
//Uses sprite and texture size instead
Object::Object(sf::RenderWindow &window, b2World *world, b2FixtureDef &fixture, sf::Sprite &sprite, sf::Vector2u &texture_size, int current_index, int body_type, int shape_type)
{
	b2BodyDef body_def;
	...
	if(shape_type == POLY_SHAPE)
	{
		poly_shape.SetAsBox( (texture_size.x / 2.0) * PIXELS_TO_METERS, (texture_size.y / 2.0) * PIXELS_TO_METERS );
		fixture.shape = &poly_shape;
	}

	else if(shape_type == CIRCLE_SHAPE)
	{
		circle_shape.m_radius = (texture_size.x / 2.0) * PIXELS_TO_METERS;
		fixture.shape = &circle_shape;
	}
